...
| Migrate                             | Fresh repository; provenance; CI/rules; stable route verification; deployment ownership transfer; catalog-only source removal; rollback     | Export, guarded ownership transfer, standalone deployment, stable-route verification, recoverable finalization, and pre-graduation rollback pass locally; remote provisioning and live acceptance remain pending |
| Provision and doctor                | Idempotent API reconciliation, read-only inspection, dry-run/apply/JSON, external resource acceptance                                       | Production and preview resources plan without writes; missing secrets, DNS visibility, and analytics permission block live acceptance                                                                            |
...
| PR previews                         | Existing Worker versions; temporary new Workers; close cleanup; staging for stateful apps                                                   | Version, temporary, and isolated stateful staging workflows pass locally; credentialed CI remains pending                                                                                                        |
...
| Accessibility and visual acceptance | Axe, keyboard, reduced motion, console/network, desktop/mobile screenshots and baselines                                                    | Shared axe and page-health checks, keyboard and reduced-motion coverage, archive isolation, and committed desktop/mobile baselines pass locally                                                                  |
| Main website                        | Published Labs link; four Functions, redirects, headers, analytics, domain, previews, and rollback verified on Workers before Pages removal | The Labs link is live; the Worker candidate lacks the existing API secrets, so Pages remains the production origin                                                                                               |
| TransitMapper                       | Current origin/main retains Vite/React and backend; consumes the shared standard; product tests pass                                        | The Vite/React application and backend remain intact in a green standard-adoption review; stable preset adoption remains pending                                                                                 |
| Organization standard               | Every active repository audited against the reusable standard; exceptions explicit                                                          | Every active repository is registered; green adoption reviews remain unmerged, and web consumers remain pinned to the unreleased preset                                                                          |
