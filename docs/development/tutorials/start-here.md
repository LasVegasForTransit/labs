...
```text
  ...
  ok    Cloudflare     wrangler is signed in; deployables: apps/home, apps/transit-funding
...
- `pnpm lab dev <slug>` starts one lab; `pnpm build` builds every lab; `pnpm run deploy` publishes
  it to Cloudflare (`.github/workflows/deploy.yml` does that on every push to `main`).
