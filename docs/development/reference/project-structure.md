...
`packages/labs-cli` owns `pnpm lab`, catalog discovery, generators, lifecycle transitions,
deployment policy, and provider target definitions. The vendored `@lasvegasfortransit/web-platform`
package owns provider reads, reconciliation, release provenance, and Worker preview guards shared
across LVBT repositories. `.github` owns contribution templates and the GitHub Actions workflows
that run those interfaces.
