...
Run `pnpm exec turbo run test:archive --filter=@lasvegasfortransit/lab-<slug>` to build and inspect
the read-only archive with live services blocked. The generated suite checks the initial project
content at desktop and mobile sizes. Extend it with the project's primary workflows as those
workflows take shape. Archive tests use `playwright.archive.config.ts`; normal preview tests exclude
that suite.
